...
// 使用 Web Crypto API 生成 UUID（Workers 环境）
function generateUUID(): string {
  // @ts-ignore - Web Crypto API
  if (typeof crypto !== 'undefined' && crypto.randomUUID) {
    // @ts-ignore
    return crypto.randomUUID();
  }
  // Fallback for environments without crypto.randomUUID
  return `u${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
}
 ...
 */
export const loginWithPrivy = async (c: Context<{ Bindings: any }>): Promise<Response> => {
  try {
    console.log('📥 Privy login request received');
    
    const body = await c.req.json();
    const { walletAddress, handle, name, avatar, privyUserId, twitterAccessToken, twitterRefreshToken } = body;
    
    if (!walletAddress) {
      return c.json({ error: { message: 'Wallet address is required' } }, 400);
    }
    ...
    if (!user) {
      ...
      let existingUser = await userRepo.findByHandle(userHandle);
      if (existingUser) {
      ...
      try {
        ...
        if (createError.message?.includes('UNIQUE constraint failed') || createError.message?.includes('Duplicate entry')) {
          ...
          user = await userRepo.create(fallbackUserData);
      ...
      if (handle && handle !== user.handle) {
        ...
        const existingUser = await userRepo.findByHandle(normalizedHandle);
      ...
      if (Object.keys(updates).length > 0) {
        try {
          console.log('📝 Updating user with:', {
            hasTwitterToken: !!updates.twitterAccessToken,
            twitterTokenLength: updates.twitterAccessToken?.length || 0,
          ...
          if (updateError.message?.includes('UNIQUE constraint failed') || updateError.message?.includes('Duplicate entry')) {
    ...
    if (!user) {
      return c.json({ error: { message: 'Failed to create or find user' } }, 500);
    }
    ...
    return c.json({ error: { message: error.message || 'Privy login failed' } }, 500);
  }
};
